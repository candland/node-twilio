 ...
 *
 * @param {Express} express: The express instance to use
 ...
 * opts.hostname: The hostname for all URIs. Default: 'localhost'.
 */
function AutoUri(express, opts) {
    var self = this;

    if(!express) {
        throw new Error('AutoUri requires an express instance');
    }
...
AutoUri.prototype.addCallback = function(method, fn, opts) {
    ...
    if(opts.expireTimeout) {
		...
		console.log("CALLBACK AT " + self.baseUri + index);
